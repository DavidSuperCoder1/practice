...
const addons:MenuItem[] = [
 ...
 const menuCategoriesDiv:number[] =[];

/*****************************************To Filter Menu Catergory**********************************************/

 const callMenuCategories = () => {
  menuAreDisplayed = false;
  const displayMenu = document.querySelector('.displayMenu') as HTMLDivElement;
  displayMenu.innerHTML= "";
  displayMenu.style.display= 'none';
  const displayMenuCate = document.querySelector('.snd-main') as HTMLDivElement;
  displayMenuCate.style.display="flex";
   ...
   if (!menuCategoriesDisplayed) {
     ...
     menuCategories.forEach((menuCate) => {
       ...
       menuCategoriesContainer.id= `${menuCate.id}`;
       const id = menuCategoriesContainer.id;
    ...
    // console.log(displayMenuCate.children.length)
     ...
     for(let i=0; i<displayMenuCate.children.length; i++){
          displayMenuCate.children[i].addEventListener('click',()=>{
...
/*****************************************To Filter Menus**********************************************/
  const filterMenu = (categoryId:number)=>{
 
    const itemIncluded = menus.filter((menuitem)=> menuitem.menuCategoryIds?.includes(categoryId));
...
/*****************************************To Display Called Menus**********************************************/
  ...
  const menulist = (itmeIncluded) =>{
    const displayMenu = document.querySelector('.displayMenu') as HTMLDivElement;
        
          
      itmeIncluded.map((menuId)=>{
       ...
       img.id = 'myImage';
       const imgUrl = menuId.assetUrl || ''; // as string ဆိုလည်းအမှားမပြ// providing default value
       img.src = imgUrl;
       ...
       const descrip_header = document.createElement('div');
       ...
       const descrip_para = document.createElement('div');
   ...
   const descrip_label = document.createElement('div') ;
