...
//    // Iterate through addons and create HTML elements
...
//      addonPrice.textContent = `Price: ${addon.price} ${addon.price === 0 ? '(Free)' : ''}`;
//      addonCard.appendChild(addonName);
//      addonCard.appendChild(addonPrice);
...
// let menuCategoriesDisplayed = false;
// const callMenuCatergories = () => {
//   if(!menuCategoriesDisplayed){
//     const displayMenuCate = document.querySelector('.snd-main');
//   menuCategories.forEach((menuCate)=>{
...
//     categoriesName.setAttribute("tabindex", "0");   //ဒီဟာထည့်မှ css ဘက်က hover ထည့်ရင် အလုပ်လုပ်မယ်။
//     categoriesName.textContent = menuCate.name;
//     //categoriesName.style.color ="white"; 
...
var callMenuCategories = function () {
    if (!menuCategoriesDisplayed) {
        var displayMenuCate_1 = document.querySelector('.snd-main');
        menuCategories.forEach(function (menuCate) {
            ...
            categoriesName.addEventListener('click', function () {
                displayMenu(menuCate.id);
            });
            menuCategoriesContainer.appendChild(categoriesName);
            displayMenuCate_1.appendChild(menuCategoriesContainer);
        });
        menuCategoriesDisplayed = true;
    }
};
var menuDisplayed = false;
var displayMenu = function (categoriesId) {
    if (!menuDisplayed) {
        var displayMenu_1 = document.querySelector('.displayMenu');
        menus.forEach(function (menu) {
            menu.menuCategoryIds.forEach(function (menuId) {
                if (categoriesId === menuId) {
                    console.log(menu.assetUrl);
                    ...
                    name_1.textContent = menuId.name;
                    descrip_header.appendChild(name_1);
                    ...
                    forDescription.appendChild(img);
                    forDescription.appendChild(description);
                    menu_card.appendChild(forDescription);
                    displayMenu_1.appendChild(menu_card);
                }
            });
        });
    }
};
// Call the function to initialize the menu categories
// callMenuCategories()
